...
it('should request and receive nearest bike station data from api', () => {
    ...
    const stationMock = require('../api/nearestStation').getNearbyStations.mockReturnValue(
        Promise.resolve(stations)
    )

    return store.dispatch(getNearbyBikeStations(coords, radius)).then(() => {
